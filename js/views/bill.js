  ...
  body.innerHTML = `
        ...
        <tbody>
          ${expRows.map((r) => `
            <tr>
              ...
              <td class="amount">${fmtMoney(r.actual)}</td>
              <td class="amount ${r.diff < 0 ? 'neg' : ''}">${r.diff === 0 ? '—' : fmtMoney(r.diff)}</td>
            </tr>`).join('')}
          <tr class="tbl__total">
            <td>合计</td>
    ...
    <div class="card">
      ...
      <table class="tbl">
        ...
        <tbody>
          ${incRows.map((r) => `<tr><td>${esc(r.cat)}</td><td class="amount amount--inc">+${fmtMoney(r.amount)}</td></tr>`).join('')}
          <tr class="tbl__total"><td>合计</td><td class="amount amount--inc">+${fmtMoney(totalInc)}</td></tr>
        </tbody>
      </table>
    </div>
  ...
  body.innerHTML = `
    ...
    <div class="card">
      ...
      <table class="tbl">
        <thead><tr><th>类别</th><th>金额</th></tr></thead>
        <tbody>
          ${catRows.map((r) => `<tr><td>${esc(r.cat)}</td><td class="amount">${fmtMoney(r.amount)}</td></tr>`).join('')}
          <tr class="tbl__total"><td>合计</td><td>${fmtMoney(totalExp)}</td></tr>
        </tbody>
      </table>
...
function txListHtml(list) {
  ...
  return Object.keys(groups).sort((a, b) => b.localeCompare(a)).map((d) => `
    <div style="margin:6px 0 2px;font-size:13px;color:var(--muted);font-weight:600">${d}</div>
    <ul class="list">${groups[d].map((t) => `
      <li class="list-item">
        ...
        <div class="amount ${t.type === 'inc' ? 'amount--inc' : ''}">${t.type === 'inc' ? '+' : '-'}${fmtMoney(t.amount)}</div>
...
export async function openRecordModal(ctx, defaultType = 'exp') {
  ...
  mask.querySelector('#m-save').onclick = async () => {
    try {
      ...
      if (!(amt > 0) || isNaN(amt)) return toast('请输入有效金额');
