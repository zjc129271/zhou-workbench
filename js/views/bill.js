...
import { renderCategoryPie, renderMonthLineChart, destroyAll } from '../charts.js';
...
async function renderMonthBill(body, ctx) {
  ...
  body.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="muted" style="font-size:13px;margin:8px 0 4px">预算可点单元格直接修改，自动保存</div>
    ...
    <div class="card">
      <div class="card__title">账单 <span class="muted" style="font-weight:400;font-size:14px">${monthTxs.length} 笔</span></div>
...
async function renderYearBill(body, ctx) {
  ...
  const expByMonth = [];
  const incByMonth = [];
  const incTxs = yearTxs.filter((t) => t.type === 'inc');
  for (let m = 1; m <= 12; m++) {
    monthLabels.push(m + '月');
    const mExp = expTxs.filter((t) => new Date(t.ts).getMonth() + 1 === m).reduce((s, t) => s + t.amount, 0);
    const mInc = incTxs.filter((t) => new Date(t.ts).getMonth() + 1 === m).reduce((s, t) => s + t.amount, 0);
    expByMonth.push(+mExp.toFixed(2));
    incByMonth.push(+mInc.toFixed(2));
  }
  const hasMonthData = expByMonth.some((v) => v > 0) || incByMonth.some((v) => v > 0);
  ...
  body.innerHTML = `
    ...
    <div class="card">
      <div class="card__title">年度汇总 <span class="muted" style="font-weight:400;font-size:14px">${fmtMonthLabel(yearSel + '-01').slice(0,5)}年支出 ${fmtMoney(totalExp)}</span></div>
      ${catRows.length ? `
      <table class="tbl">
        <thead><tr><th>类别</th><th>金额</th></tr></thead>
        <tbody>
    ...
    ${hasMonthData ? `
    <div class="card">
      <div class="card__title">月支出统计 <span class="muted" style="font-weight:400;font-size:14px">全年合计 ${fmtMoney(expByMonth.reduce((s, v) => s + v, 0))}</span></div>
      <div class="chart-wrap" style="height:260px"><canvas id="lineExp"></canvas></div>
    </div>
    <div class="card">
      <div class="card__title">月收入统计 <span class="muted" style="font-weight:400;font-size:14px">全年合计 ${fmtMoney(incByMonth.reduce((s, v) => s + v, 0))}</span></div>
      <div class="chart-wrap" style="height:260px"><canvas id="lineInc"></canvas></div>
    </div>
    ` : ''}
  `;
  ...
  if (hasMonthData) {
    try {
      const expCanvas = body.querySelector('#lineExp');
      if (expCanvas) renderMonthLineChart(expCanvas, monthLabels, expByMonth, '#12915a', '月支出');
      const incCanvas = body.querySelector('#lineInc');
      if (incCanvas) renderMonthLineChart(incCanvas, monthLabels, incByMonth, '#e9c46a', '月收入');
    }
  ...
  const mask = openModal(`
    <div class="modal-card">
      <div class="modal-head">
        ...
        <button class="icon-btn" id="m-close" aria-label="关闭" style="font-size:19px">✕</button>
      </div>
      <div class="field"><label>金额</label><input class="input" id="m-amt" type="number" inputmode="decimal" placeholder="0.00" /></div>
      <div class="muted" style="font-size:13px;margin-top:-4px">可输入负数，负数将直接参与统计（如负支出冲减总额）</div>
