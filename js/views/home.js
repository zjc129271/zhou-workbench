...
export async function render(container, ctx) {
  ...
  container.innerHTML = `
    ...
    <section class="section">
      ...
      <div class="dash">
        ${statCard(`${dailyDone}<span class="muted" style="font-size:14px">/${dailyTasks.length}</span>`, '每日任务完成')}
        ${statCard(`${weeklyDone}<span class="muted" style="font-size:14px">/${weeklyTasks.length}</span>`, '本周任务完成')}
